...
function equals(actual, expected) {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!vapors_1.vapors.utils.isHexString(actual)) {
            return false;
        }
        actual = vapors_1.vapors.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        var actualAddress = vapors_1.vapors.utils.getAddress(actual);
        var expectedAddress = vapors_1.vapors.utils.getAddress(expected);
...
function getValues(object, named) {
    ...
    switch (object.type) {
        ...
        case 'string':
            return object.value;
        case 'buffer':
            return vapors_1.vapors.utils.arrayify(object.value);
        case 'tuple':
...
describe('ABI Coder Encoding', function () {
    var coder = vapors_1.vapors.utils.defaultAbiCoder;
...
describe('Test Contract Events', function () {
    ...
    tests.forEach(function (test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function () {
            this.timeout(120000);
            var iface = new vapors_1.vapors.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function () {
            ...
            test.normalizedValues.forEach(function (expected, index) {
                if (test.indexed[index]) {
                    assert_1.default.ok((vapors_1.vapors.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                }
                else {
...
describe('Test Interface Signatures', function () {
    ...
    tests.forEach(function (test) {
        it('derives the correct signature - ' + test.name, function () {
            var iface = new vapors_1.vapors.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function () {
        var iface = new vapors_1.vapors.utils.Interface(["function transfer(address from, uint amount)"]);
        [
            "transfer",
    ...
    // See: https://github.com/vaporsjs/vapors.js/issues/370
    it('parses transaction function', function () {
        ...
        var tx = vapors_1.vapors.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function () {
    ...
    it('encodes zero', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: vapors_1.vapors.constants.Zero }
        ];
    ...
    it('encodes one', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: vapors_1.vapors.constants.One }
        ];
        var expected = oneHex;
        ['uint8', 'uint256', 'int8', 'int256'].forEach(function (type) {
            ZeroValues.forEach(function (value) {
    ...
    it('encodes negative one', function () {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: vapors_1.vapors.constants.NegativeOne }
        ];
        var expected = maxHex;
    ...
    it('encodes full uint8 range', function () {
        for (var i = 0; i < 256; i++) {
            ...
            expected += vapors_1.vapors.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function () {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += vapors_1.vapors.utils.hexlify(256 + i).substring(2);
            }
            else {
...
describe('Test Filters', function () {
    ...
    function doTest(test) {
        it(test.name, function () {
            var iface = new vapors_1.vapors.utils.Interface([test.signature]);
    ...
    var Tests = [
        ...
        // See: https://github.com/vaporsjs/vapors.js/issues/305
        {
        ...
        // https://vaporscan.io/tx/0x820cc57bc77be44d8f4f024a18e18f64a8b6e62a82a3d7897db5970dbe181ba1
        {
...
describe("Test ParamType Parser", function () {
    ...
    Tests.forEach(function (test) {
        it("allows correct modifiers " + JSON.stringify(test.type), function () {
            var paramType = vapors_1.vapors.utils.ParamType.from(test.type);
