...
import { loadTests, TestCase } from "@vaporsproject/testcases";
...
describe('Test JSON Wallets', function() {
    ...
    tests.forEach(function(test) {
        it(('decrypts wallet - ' + test.name), async function() {
            ...
            if (test.hasAddress) {
                assert.ok((vapors.utils.getJsonWalletAddress(test.json) !== null),
                    'detect encrypted JSON wallet');
            }

            const wallet = await vapors.Wallet.fromEncryptedJson(test.json, test.password);
...
describe('Test Transaction Signing and Parsing', function() {
    function checkTransaction(parsedTransaction: any, test: TestCase.SignedTransaction): any {
        ...
        ['nonce', 'gasLimit', 'gasPrice', 'to', 'value', 'data'].forEach((key: (keyof TestCase.SignedTransaction)) => {
            ...
            if ([ "gasLimit", "gasPrice", "value"].indexOf(key) >= 0) {
                assert.ok((vapors.BigNumber.isBigNumber(value)),
    ...
    tests.forEach((test) => {
        it(('parses and signs transaction - ' + test.name), function() {
            ...
            assert.equal(parsedTransaction.from, vapors.utils.getAddress(test.accountAddress),
                'computed from');
            ...
            (function() {
                ...
                assert.equal(vapors.utils.serializeTransaction(transaction, signature), test.signedTransaction,
            ...
            (function() {
                ...
                let signature = signDigest(vapors.utils.keccak256(unsignedTx));
                assert.equal(vapors.utils.serializeTransaction(transaction, signature), test.signedTransactionChainId5,
...
describe("Wallet Errors", function() {
    ...
    it("fails on mnemonic/address mismatch", function() {
        assert.throws(() => {
            const wallet = new vapors.Wallet(<any>{
                privateKey: "0x6a73cd9b03647e83ef937888a5258a26e4c766dbf41ddd974f15e32d09cfe9c0",
