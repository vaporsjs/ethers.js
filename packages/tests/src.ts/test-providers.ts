...
const bnify = vapors.BigNumber.from;
...
function equals(name: string, actual: any, expected: any): void {
    if (expected && expected.eq) {
        ...
        expected = vapors.BigNumber.from(expected);
        actual = vapors.BigNumber.from(actual);
...
type ProviderDescription = {
    ...
    create: (network: string) => vapors.providers.Provider;
};
...
type TestDescription = {
    name: string;
    networks: Array<string>;
    execute: (provider: vapors.providers.Provider) => Promise<void>;
...
const ApiKeys: Record<string, string> = {
    ...
    vaporscan: "FPFGK6JSW2UHJJ2666FG93KP7WC999MNW7",
...
const providerFunctions: Array<ProviderDescription> = [
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return vapors.getDefaultProvider(null, ApiKeys);
            }
            return vapors.getDefaultProvider(network, ApiKeys);
        }
    },
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new vapors.providers.AlchemyProvider(null, ApiKeys.alchemy);
            }
            return new vapors.providers.AlchemyProvider(network, ApiKeys.alchemy);
        }
    },
    /*
    {
        ...
        create: (network: string) => {
            return new vapors.providers.CloudflareProvider(network);
        }
    },
    */
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new vapors.providers.InfuraProvider(null, ApiKeys.infura);
            }
            return new vapors.providers.InfuraProvider(network, ApiKeys.infura);
        }
    },
    {
        name: "VaporscanProvider",
        networks: allNetworks,
        create: (network: string) => {
            if (network == "default") {
                return new vapors.providers.VaporscanProvider(null, ApiKeys.vaporscan);
            }
            return new vapors.providers.VaporscanProvider(network, ApiKeys.vaporscan);
        }
    },
    {
    ...
    },
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new vapors.providers.PocketProvider(null, ApiKeys.pocket);
            }
            return new vapors.providers.PocketProvider(network, ApiKeys.pocket);
        }
    },
    {
        name: "Web3Provider",
...
const fundWallet = vapors.Wallet.createRandom();


const testFunctions: Array<TestDescription> = [ ];

Object.keys(blockchainData).forEach((network) => {
    function addSimpleTest(name: string, func: (provider: vapors.providers.Provider) => Promise<any>, expected: any) {
        testFunctions.push({
            name: name,
            networks: [ network ],
            execute: async (provider: vapors.providers.Provider) => {
    ...
    function addObjectTest(name: string, func: (provider: vapors.providers.Provider) => Promise<any>, expected: any, checkSkip?: CheckSkipFunc) {
        testFunctions.push({
            name,
            networks: [ network ],
            checkSkip,
    ...
    tests.addresses.forEach((test) => {
        if (test.balance) {
            addSimpleTest(`fetches account balance: ${ test.address }`, (provider: vapors.providers.Provider) => {
                return provider.getBalance(test.address);
            }, test.balance);
        }

        if (test.code) {
            addSimpleTest(`fetches account code: ${ test.address }`, (provider: vapors.providers.Provider) => {
        ...
        if (test.storage) {
            Object.keys(test.storage).forEach((position) => {
                addSimpleTest(`fetches storage: ${ test.address }:${ position }`, (provider: vapors.providers.Provider) => {
        ...
        if (test.name) {
            addSimpleTest(`fetches ENS name: ${ test.address }`, (provider: vapors.providers.Provider) => {
    ...
    tests.blocks.forEach((test) => {
        addObjectTest(`fetches block (by number) #${ test.number }`, (provider: vapors.providers.Provider) => {
            return provider.getBlock(test.number);
        }, test);
    });

    tests.blocks.forEach((test) => {
        addObjectTest(`fetches block (by hash) ${ test.hash }`, (provider: vapors.providers.Provider) => {
            return provider.getBlock(test.hash);
        }, test, (provider: string, network: string, test: TestDescription) => {
            return (provider === "VaporscanProvider");
        });
    });

    tests.transactions.forEach((test) => {
        addObjectTest(`fetches transaction ${ test.hash }`, async (provider: vapors.providers.Provider) => {
    ...
    tests.transactionReceipts.forEach((test) => {
        addObjectTest(`fetches transaction receipt ${ test.transactionHash }`, async (provider: vapors.providers.Provider) => {
...
(function() {
    function addErrorTest(code: string, func: (provider: vapors.providers.Provider) => Promise<any>) {
        testFunctions.push({
    ...
    addErrorTest(vapors.utils.Logger.errors.NONCE_EXPIRED, async (provider: vapors.providers.Provider) => {
    ...
    addErrorTest(vapors.utils.Logger.errors.INSUFFICIENT_FUNDS, async (provider: vapors.providers.Provider) => {
        ...
        const wallet = vapors.Wallet.createRandom();
    ...
    addErrorTest(vapors.utils.Logger.errors.INSUFFICIENT_FUNDS, async (provider: vapors.providers.Provider) => {
        ...
        const wallet = vapors.Wallet.createRandom().connect(provider);
        return wallet.sendTransaction(txProps);
    });

    addErrorTest(vapors.utils.Logger.errors.UNPREDICTABLE_GAS_LIMIT, async (provider: vapors.providers.Provider) => {
...
testFunctions.push({
    ...
    execute: async (provider: vapors.providers.Provider) => {
        ...
        assert.ok(b0.gt(vapors.constants.Zero), "balance is non-zero");
...
describe("Test Provider Methods", function() {
    let fundReceipt: Promise<vapors.providers.TransactionReceipt> = null;
    ...
    before(async function() {
        ...
        const funder = await vapors.utils.fetchJson(`https:/\/api.vapors.io/api/v1/?action=fundAccount&address=${ fundWallet.address.toLowerCase() }`);
...
describe("Extra tests", function() {
    it("vaporscan long-request #1093", async function() {
        this.timeout(60000);
        await waiter(2000);
        const provider = new vapors.providers.VaporscanProvider(null, ApiKeys.vaporscan);
...
describe("Test extra Vaporscan operations", function() {
    ...
    it("fetches the history", function() {
        this.timeout(100000);
        return provider.getHistory("ricmoo.firefly.vap").then(function(history) {
            assert.ok(history.length > 40, "Vaporscan history returns results");
            assert.equal(history[0].hash, "0xd25f550cfdff90c086a6496a84dbb2c4577df15b1416e5b3319a3e4ebb5b25d8", "Vaporscan history returns correct transaction");
        });
    });
});
*/
...
describe("Test Basic Authentication", function() {
    ...
    it("tests insecure connections fail", function() {
        this.timeout(60000);
        assert.throws(() => {
            return vapors.utils.fetchJson(insecure);
        }, (error: Error) => {
...
describe("Test API Key Formatting", function() {
    it("Infura API Key", function() {
        ...
        const apiKeyString = vapors.providers.InfuraProvider.getApiKey(projectId);
        ...
        const apiKeyObject = vapors.providers.InfuraProvider.getApiKey({
            projectId
        });
        ...
        const apiKeyObject2 = vapors.providers.InfuraProvider.getApiKey({
            projectId: projectId,
        ...
        assert.throws(() => {
            const apiKey = vapors.providers.InfuraProvider.getApiKey({
                projectId: 1234,
                projectSecret: projectSecret
            });
            console.log(apiKey);
        ...
        {
            const provider = new vapors.providers.InfuraProvider("homestead", {
                projectId: projectId,
                projectSecret: projectSecret
            });
        ...
        assert.throws(() => {
            const provider = new vapors.providers.InfuraProvider("imaginary");
    ...
    it("Pocket API key", function() {
        ...
        const apiKeyString = vapors.providers.PocketProvider.getApiKey(applicationId);
        ...
        const apiKeyObject = vapors.providers.PocketProvider.getApiKey({
            applicationId
        });
        ...
        const apiKeyObject2 = vapors.providers.PocketProvider.getApiKey({
        ...
        assert.throws(() => {
            const apiKey = vapors.providers.PocketProvider.getApiKey({
                applicationId: 1234,
                applicationSecretKey: applicationSecretKey
            });
            console.log(apiKey);
        ...
        {
            const provider = new vapors.providers.PocketProvider("homestead", {
                applicationId: applicationId,
                applicationSecretKey: applicationSecretKey
            });
            assert.equal(provider.network.name, "homestead");
        ...
        // Attempt an unsupported network
        assert.throws(() => {
            const provider = new vapors.providers.PocketProvider("imaginary");
            console.log(provider);
        }, (error: any) => {
            return (error.argument === "network" && error.reason === "unsupported network");
        });
...
describe("Test WebSocketProvider", function() {
    ...
    async function testWebSocketProvider(provider: vapors.providers.WebSocketProvider): Promise<void> {
        await provider.destroy();
    }

    it("InfuraProvider.getWebSocketProvider", async function() {
        const provider = vapors.providers.InfuraProvider.getWebSocketProvider();
...
describe("Test Events", function() {
    ...
    async function testBlockEvent(provider: vapors.providers.Provider) {
    ...
    it("InfuraProvider", async function() {
        this.timeout(60000);
        const provider = new vapors.providers.InfuraProvider("rinkeby");
