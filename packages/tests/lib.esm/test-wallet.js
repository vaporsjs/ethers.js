...
describe('Test JSON Wallets', function () {
    ...
    tests.forEach(function (test) {
        it(('decrypts wallet - ' + test.name), function () {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (test.hasAddress) {
                    assert.ok((vapors.utils.getJsonWalletAddress(test.json) !== null), 'detect encrypted JSON wallet');
                }
                const wallet = yield vapors.Wallet.fromEncryptedJson(test.json, test.password);
                ...
                // Test connect
                {
                    const provider = new vapors.providers.VaporscanProvider();
                ...
                {
                    const wallet2 = new vapors.Wallet(wallet._signingKey());
                ...
                if (test.name === "life") {
                    const wallet2 = vapors.Wallet.fromEncryptedJsonSync(test.json, test.password);
                    assert.equal(wallet2.privateKey, test.privateKey, 'generated correct private key - ' + wallet2.privateKey);
                }
                if (test.mnemonic) {
    ...
    ['one', 'two', 'three'].forEach(function (i) {
        ...
        let wallet = vapors.Wallet.createRandom({ path: "m/56'/82", extraEntropy: utils.randomHexString('test-' + i, 32) });
        it('encrypts and decrypts a random wallet - ' + i, function () {
            this.timeout(1200000);
            return wallet.encrypt(password).then((json) => {
                return vapors.Wallet.fromEncryptedJson(json, password).then((decryptedWallet) => {
...
describe('Test Transaction Signing and Parsing', function () {
    function checkTransaction(parsedTransaction, test) {
        ...
        ['nonce', 'gasLimit', 'gasPrice', 'to', 'value', 'data'].forEach((key) => {
            ...
            if (["gasLimit", "gasPrice", "value"].indexOf(key) >= 0) {
                assert.ok((vapors.BigNumber.isBigNumber(value)), 'parsed into a big number - ' + key);
                ...
                value = vapors.utils.hexlify(value);
                if (!expected || expected === '0x') {
                    expected = '0x00';
                }
            }
    ...
    tests.forEach((test) => {
        it(('parses and signs transaction - ' + test.name), function () {
            this.timeout(120000);
            let signingKey = new vapors.utils.SigningKey(test.privateKey);
            let signDigest = signingKey.signDigest.bind(signingKey);
            // Legacy parsing unsigned transaction
            checkTransaction(vapors.utils.parseTransaction(test.unsignedTransaction), test);
            let parsedTransaction = vapors.utils.parseTransaction(test.signedTransaction);
            let transaction = checkTransaction(parsedTransaction, test);
            // Legacy signed transaction ecrecover
            assert.equal(parsedTransaction.from, vapors.utils.getAddress(test.accountAddress), 'computed from');
            ...
            (function () {
                let unsignedTx = vapors.utils.serializeTransaction(transaction);
                ...
                let signature = signDigest(vapors.utils.keccak256(unsignedTx));
                assert.equal(vapors.utils.serializeTransaction(transaction, signature), test.signedTransaction, 'signs transaction (legacy)');
            })();
            // EIP155
            // EIP-155 parsing unsigned transaction
            let parsedUnsignedTransactionChainId5 = vapors.utils.parseTransaction(test.unsignedTransactionChainId5);
            ...
            // EIP-155 fields
            let parsedTransactionChainId5 = vapors.utils.parseTransaction(test.signedTransactionChainId5);
            ...
            (function () {
                ...
                let signature = signDigest(vapors.utils.keccak256(unsignedTx));
                assert.equal(vapors.utils.serializeTransaction(transaction, signature), test.signedTransactionChainId5, 'signs transaction (eip155)');
            })();
        });
    });
    tests.forEach((test) => {
...
describe('Test Signing Messages', function () {
    let tests = [
        ...
        // See: https://github.com/vaporsjs/vapors.js/issues/80
        {
            ...
            message: vapors.utils.arrayify('0x47173285a8d7341e5e972fc677286384f802f8ef42a5ec5f03bbfa254cb01fad'),
        ...
        // See: https://github.com/vaporsjs/vapors.js/issues/85
        {
            ...
            message: vapors.utils.arrayify(vapors.utils.id('0x7f23b5eed5bc7e89f267f339561b2697faab234a2')),
    ...
    tests.forEach(function (test) {
        it(('verifies a message "' + test.name + '"'), function () {
            this.timeout(120000);
            let address = vapors.utils.verifyMessage(test.message, test.signature);
    ...
    tests.forEach(function (test) {
        it(('hashes a message "' + test.name + '"'), function () {
            this.timeout(120000);
            let hash = vapors.utils.hashMessage(test.message);
...
describe("Serialize Transactions", function () {
    it("allows odd-length numeric values", function () {
        vapors.utils.serializeTransaction({
            gasLimit: "0x1",
            gasPrice: "0x1",
            value: "0x1"
        });
...
describe("Wallet Errors", function () {
    it("fails on privateKey/address mismatch", function () {
        assert.throws(() => {
            const wallet = new vapors.Wallet({
    ...
    it("fails on from mismatch", function () {
        const wallet = new vapors.Wallet("0x6a73cd9b03647e83ef937888a5258a26e4c766dbf41ddd974f15e32d09cfe9c0");
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (error.code === vapors.utils.Logger.errors.INVALID_ARGUMENT && error.argument === "transaction.from") {
                    resolve(true);
                    return;
                }
            }
